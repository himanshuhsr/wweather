...
import './App.css';
import { Card, Col, Container, FormControl, InputGroup, Row, Button,Alert } from 'react-bootstrap';
...
const App = () => {
  ...
  const fetchData = async() =>{
    ...
    setLoader(true);
    if(query.length < 3){
      setError('Please enter correct city or zip code.');
      setLoader(false);
    }else{
      try{
        const response = await axios.get(`http://api.weatherapi.com/v1/current.json?key=${process.env.REACT_APP_API_KEY}&q=${query}&aqi=yes`);
        ...
        <Col lg={9} md={10}>
          <Card className='p-4 shadow weather-data'>
            ...
            <Container>
              ...
              <span className='text-blue d-block text-center mt-2'><i className='bi bi-clock'></i> {data.location.tz_id}</span>
                <div className='my-4'>
